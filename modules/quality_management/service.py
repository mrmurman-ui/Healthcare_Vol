    ...
    with tab1:
        ...
        with st.expander("➕ " + _t("เพิ่มตัวชี้วัด", "Add Indicator")):
            with st.form("add_indicator"):
                ...
                is_thai = _t("th", "en") == "th"
                cat_options = [CATEGORY_LABELS_TH.get(c, c) for c in INDICATOR_CATEGORIES] if is_thai else INDICATOR_CATEGORIES
                cat_display = col1.selectbox(_t("หมวดหมู่", "Category"), cat_options)
                cat = INDICATOR_CATEGORIES[[CATEGORY_LABELS_TH.get(c, c) for c in INDICATOR_CATEGORIES].index(cat_display)] if is_thai else cat_display
                target = col2.number_input(_t("ค่าเป้าหมาย", "Target Value"), min_value=0.0, value=80.0)
                unit = col1.text_input(_t("หน่วย (เช่น %)", "Unit (e.g. %)"))
                period_options = [PERIOD_LABELS_TH.get(p, p) for p in PERIODS] if is_thai else PERIODS
                period_display = col2.selectbox(_t("ช่วงเวลา", "Period"), period_options)
                period = PERIODS[[PERIOD_LABELS_TH.get(p, p) for p in PERIODS].index(period_display)] if is_thai else period_display
