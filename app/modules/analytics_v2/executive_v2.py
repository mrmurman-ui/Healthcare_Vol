  ...
  <div class="insight-text">{ins['text']}</div>
    ...
    with col2:
        ...
        if g_data:
            ...
            _gmap = _gl()
            fig_g = px.pie(
                names=[_gmap.get(k, k) for k in g_data.keys()],
    ...
    if gis:
        ...
        _MAP_EN = ["Population Density", "Elderly Density", "Disabled Density", "Bedridden Density"]
        _MAP_TH = ["ความหนาแน่นประชากร", "ความหนาแน่นผู้สูงอายุ", "ความหนาแน่นผู้พิการ", "ความหนาแน่นติดเตียง"]
        _is_th = st.session_state.get("lang", "TH") == "TH"
        _map_display = _MAP_TH if _is_th else _MAP_EN
        _map_idx = st.selectbox(
            _t("ชั้นข้อมูล", "Map Layer"),
            range(len(_map_display)),
            format_func=lambda i: _map_display[i],
            key="gis_layer",
        )
        map_type = _MAP_EN[_map_idx]
