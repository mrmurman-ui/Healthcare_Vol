...
def be_date_input(
    ...
    """
    Wrapper around st.date_input that appends a พ.ศ. hint when in Thai mode.
    The underlying widget works with AD dates (Python date objects).
    The returned value is always a Python date in AD — ready for DB storage.

    Usage:
        visit_date = be_date_input("วันที่เยี่ยม", value=date.today())
        # visit_date is a Python date (AD), safe to save directly to DB
        # Displayed year in widget title shows พ.ศ. hint for Thai users
    """
    if value is None:
    ...
    if is_thai():
        be_year = ad_to_be_year(value.year) if value else ad_to_be_year(2026)
        hint = f" (พ.ศ. {be_year})"
        display_label = label + hint
    else:
        display_label = label

    kwargs = dict(label=display_label, value=value)
    if key:
        kwargs["key"] = key
    if help:
        kwargs["help"] = help
